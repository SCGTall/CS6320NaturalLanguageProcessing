...
class IMDBDataset(Dataset):
    ...
    def __init__(self, input_directory, tokenizer, apply_cleaning, max_tokenization_length,
        ...
        self.tweets = []
        self.tweets_id = []
        for line in open(input_directory, 'r'):
           cur_tweet = json.loads(line)
           self.tweets.append(cur_tweet['text'])
           self.tweets_id.append(cur_tweet['id'])
        #self.positive_path = input_directory
        #self.positive_files = [f for f in os.listdir(self.positive_path)
                              # if os.path.isfile(os.path.join(self.positive_path, f))]
        self.num_examples = len(self.tweets)
    ...
    def pre_tokenize_and_encode_examples(self):
        ...
        """
        if not os.path.exists('tokenized_and_encoded'):
            os.mkdir('tokenized_and_encoded')

            # Clean & tokenize positive reviews
            for i in trange(self.num_examples, desc='Tokenizing & Encoding Reviews',
                            leave=True):
                example = self.tweets[i]
               # with open(os.path.join(self.positive_path, file), mode='r', encoding='unicode_escape') as f:
                  #  example = f.read()
                ...
                this_file = open(os.path.join('tokenized_and_encoded', str(i)), mode='wb')
                pickle.dump(obj=example, file=this_file)
        else:
            logging.warning('Tokenized reviews directory already exists!')
        ...
        #         with open(os.path.join(self.negative_path, file), mode='r', encoding='unicode_escape') as f:
        #             example = f.read()
    ...
    def __getitem__(self, index):
        if index < self.num_examples:
            ...
            with open(os.path.join('tokenized_and_encoded', str(index)), mode='rb') as f:
