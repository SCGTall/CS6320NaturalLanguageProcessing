...
import logging
import torch
from utils.data_utils_test import get_features
...
def test(model, iterator, criterion, device, result, include_bert_masks=True):
    """
    ...
    with torch.no_grad():
        for batch in iterator:
            ...
            # Calculate loss and accuracy
            #result_string = predictions +"#"+labels
        
            loss = criterion(predictions, labels)
            cur_result = binary_accuracy(predictions, labels)
            cur_result =  cur_result.numpy()
            result.append(cur_result[0])
